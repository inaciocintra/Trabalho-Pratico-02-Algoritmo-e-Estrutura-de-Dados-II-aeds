...
class Personagem {
 ...
 class Ex05{
    public static  void ordenacaoSelecao(ArrayList<Personagem> personagens){
        ...
        for (int i = 0; i < personagens.size()-1; i++) {
            ...
            for (int j = i + 1; j < personagens.size(); j++) {
                if ((personagens.get(j).getName().compareTo(personagens.get(min_idx).getName()))<0 ) {
                    min_idx = j;
                }
            }
    
            // Troque o menor encontrado pelo primeiro elemento
            Personagem temp = personagens.get(min_idx);
            personagens.set(min_idx, personagens.get(i));
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("matricula_selecao.txt"))) {
            writer.write("803040\t" + numComparacoes + "\t" + numMovimentacoes + "\t" + tempoExecucao);
        } catch (IOException e) {
            e.printStackTrace();
        }
