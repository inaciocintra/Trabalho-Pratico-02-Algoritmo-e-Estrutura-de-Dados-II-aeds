...
//quickparcial
public static void quickparcial(Personagem[] personagens, int esq, int dir, int k) {
    int i = esq, j = dir;
    //define o pivô
    Personagem pivo = personagens[(esq + dir) / 2];

    while (i <= j) {
        //compara pelas casas primeiro e usa o nome como critério de desempate
        while (comparePersonagens(personagens[i], pivo) < 0) {
            i++;
        }
        while (comparePersonagens(personagens[j], pivo) > 0) {
            j--;
        }
        if (i <= j) {
    ...
    //chama recursivamente, ajustando para o limite k
    if (esq < j) {
        quickparcial(personagens, esq, j, k);
    }
    if (i < dir && i < k) {
        quickparcial(personagens, i, dir, k);
    }
}

//metodo auxiliar para comparar dois personagens levando em conta a casa e o nome
private static int comparePersonagens(Personagem a, Personagem b) {
    int houseCompare = a.getHouse().compareTo(b.getHouse());
    if (houseCompare == 0) {
...
//metodo swap para trocar dois personagens de posição
private static void swap(Personagem[] personagens, int i, int j) {
    ...
    int k = 10;
    int esq=0;
    quickparcial(personagens,esq,j-1,k);
    for(int i = 0; i<k; i++){
