...
class Personagem {
 ...
 class Ex11{ 
    ...
    public static void countingSort(Personagem[] personagens, int n) {
        if (n <= 0) {
            return; //nada pra fazer se o array estiver vazio ou se n não for válido
        }
    
        //encontrar o menor e maior ano de nascimento
        int menorAno = personagens[0].getYearOfBirth();
        int maiorAno = personagens[0].getYearOfBirth();
        for (int i = 1; i < n; i++) {
            int ano = personagens[i].getYearOfBirth();
            if (ano < menorAno) {
                menorAno = ano;
            }
            if (ano > maiorAno) {
        ...
        //intervalo (range) do array de contagem
        int range = maiorAno - menorAno + 1;
        int[] count = new int[range];
    
        //contar os anos de nascimento
        for (int i = 0; i < n; i++) {
            int index = personagens[i].getYearOfBirth() - menorAno;
            count[index]++;
        }
    
        //transformar `count` em contagem cumulativa
        for (int i = 1; i < range; i++) {
            count[i] += count[i - 1];
        }
    
        //criar o array de personagens ordenados
        Personagem[] ordenado = new Personagem[n];
        for (int i = n - 1; i >= 0; i--) {
            int ano = personagens[i].getYearOfBirth();
            int index = ano - menorAno;
            count[index]--;
            ordenado[count[index]] = personagens[i];
        }
    
        //copiar os elementos ordenados de volta para `personagens`
        for (int i = 0; i < n; i++) {
            personagens[i] = ordenado[i];
        }
    
        //ordenar por nome dentro dos grupos com o mesmo ano de nascimento
        ordenarPorNomeDentroDoMesmoAno(personagens, n);
    }
    
    private static void ordenarPorNomeDentroDoMesmoAno(Personagem[] personagens, int n) {
        int start = 0;
        while (start < n) {
            int ano = personagens[start].getYearOfBirth();
            int end = start + 1;
    
            //encontrar o subarray com o mesmo ano de nascimento
            while (end < n && personagens[end].getYearOfBirth() == ano) {
                end++;
            }
    
            //ordenar o subarray por nome usando ordenação por inserção
            ordenarPorInsercaoPorNome(personagens, start, end);
    
            start = end; //avançar para o próximo grupo com um ano diferente
        }
    }
    
    private static void ordenarPorInsercaoPorNome(Personagem[] personagens, int start, int end) {
        for (int i = start + 1; i < end; i++) {
            Personagem chave = personagens[i];
            int j = i - 1;
    
            //comparar o nome dos personagens
            while (j >= start && personagens[j].getName().compareTo(chave.getName()) > 0) {
                personagens[j + 1] = personagens[j];
                j--;
            }
    
            personagens[j + 1] = chave;
        }
    }
